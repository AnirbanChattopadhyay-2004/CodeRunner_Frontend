...
export default function Row({
  ...
  return (
    <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
      ...
      <td className="px-6 py-4">{sourcecode.substring(0, 100)}</td>
      <td className="px-6 py-4">{stdinput}</td>
      <td
        className={`px-6 py-4 ${
          status.charAt(0) == "A" ? "text-green-600" : "text-rose-600"
        }`}
      >
        {status}
      </td>
      <td className="px-6 py-4 ">{convertUTCToIST(created)}</td>
      <td className="px-6 py-4 ">{stdoutput}</td>
      <td
        className="px-6 py-4 cursor-pointer hover:text-red-500 "
        onClick={() => {
      ...
      <td
        className="px-6 py-4 cursor-pointer hover:text-green-500"
        onClick={() => {
          navigator.clipboard.writeText(
            "https://code-runner-frontend-ruby.vercel.app/page/" + id
          );
